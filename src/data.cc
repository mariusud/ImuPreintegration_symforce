...
void visualizeTrajectory(const sym::Valuesd &optimized_values, const std::vector<GpsMeasurement> &gps_measurements, int NUM_FACTORS) {
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr optimized_imu_cloud(new pcl::PointCloud<pcl::PointXYZ>());
  pcl::PointCloud<pcl::PointXYZ>::Ptr gps_cloud(new pcl::PointCloud<pcl::PointXYZ>());

  for (size_t i = 0; i < NUM_FACTORS - 1; i++) {
    sym::Pose3d pose = optimized_values.At<sym::Pose3d>(sym::Keys::POSE.WithSuper(i));
    pcl::PointXYZ point;
  ...
  for (size_t i = 1; i < gps_measurements.size() - 1; i++) {
    ...
    viewer.addLine(start_point, end_point, 0, 1, 0, "gps line" + std::to_string(i));
  }

  viewer.addText("- IMU Trajectory: White", 800, 600 - 20, 12, 1.0, 1.0, 1.0, "imu_legend");
  viewer.addText("- GPS Trajectory: Green", 800, 600 - 40, 12, 0.0, 1.0, 0.0, "gps_legend");
...
void visualizeData(const std::vector<ImuMeasurement> &imu_measurements, const std::vector<GpsMeasurement> &gps_measurements, size_t num_to_visualize) {
  ...
  for (size_t i = 0; i < gps_to_display; ++i) {
    const auto &gps = gps_measurements[i];
    std::cout << std::fixed << std::setprecision(4) << std::setw(10) << gps.time << std::setw(15) << gps.position.x() << std::setw(15) << gps.position.y() << std::setw(15) << gps.position.z() << "\n";
  }

  std::cout << "\nSample IMU Measurements:\n";
  std::cout << std::setw(10) << "Time" << std::setw(15) << "Accel (X)" << std::setw(15) << "Accel (Y)" << std::setw(15) << "Accel (Z)" << std::setw(15) << "AngVel (X)" << std::setw(15) << "AngVel (Y)"
            << std::setw(15) << "AngVel (Z)"
            << "\n";

  for (size_t i = 0; i < imu_to_display; ++i) {
    const auto &imu = imu_measurements[i];
    std::cout << std::fixed << std::setprecision(4) << std::setw(10) << imu.time << std::setw(15) << imu.accelerometer.x() << std::setw(15) << imu.accelerometer.y() << std::setw(15)
              << imu.accelerometer.z() << std::setw(15) << imu.gyroscope.x() << std::setw(15) << imu.gyroscope.y() << std::setw(15) << imu.gyroscope.z() << "\n";
  }
}